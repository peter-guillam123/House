...
async function runSearch(isFresh) {
  ...
  const { failed } = await runSourceFetches(myToken, sourceKeys, fetchOpts, partyIdSet);
  if (myToken !== state.searchToken) return;
  ...
  if (failed.length) {
    ...
    if (partyIdSet) {
      const scanned = state.offsets.spoken + state.offsets.wq + state.offsets.ws + state.offsets.committee;
      setStatus(haveMore
        ? `No matches from ${state.party.name} in the first ${scanned} hits. Try "Load more" or broaden filters.`
        : `No matches from ${state.party.name} across all ${scanned} hits for this window. Try broadening the date range or filters.`);
    } else {
    ...
    const scanned = state.offsets.spoken + state.offsets.wq + state.offsets.ws + state.offsets.committee;
    setStatus(haveMore
      ? `Showing ${state.items.length} ${state.party.name} results from the first ${scanned} of ${totalAvailable} hits.`
      : `Showing ${state.items.length} ${state.party.name} results across all ${scanned} hits for this window.`);
  } else {
    setStatus(`Showing ${state.items.length} of ${totalAvailable} results.`);
  }
...
function setStatusWithRetry(failedKeys, itemCount) {
  ...
  const msg = itemCount > 0
    ? `Showing ${itemCount} results. ${list} didn't respond — Parliament's API gets flaky on common terms.`
