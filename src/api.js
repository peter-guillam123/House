...
// Parliament's endpoints throw transient 500s and slow cold-starts on
// less-common queries ('united ireland' has been observed taking 16s
// cold, sub-100ms warm). One retry with backoff catches transient
// failures; an AbortController-driven timeout stops a stalled upstream
// from blocking the page. First attempt has a tight timeout to fail
// fast on a genuinely dead endpoint; the retry is patient enough to
// catch responses that were slow, not broken.
const FETCH_TIMEOUT_MS       = 12_000;
const FETCH_TIMEOUT_RETRY_MS = 20_000;
const RETRY_DELAY_MS         = 600;

async function getJson(url) {
  for (let attempt = 0; attempt < 2; attempt++) {
    const timeout = attempt === 0 ? FETCH_TIMEOUT_MS : FETCH_TIMEOUT_RETRY_MS;
    ...
    try {
      ...
      if (isAbort) throw new Error(`timed out after ${timeout / 1000}s on ${url}`);
      throw e;
    } finally {
      clearTimeout(timer);
    }
